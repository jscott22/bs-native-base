...
type iconType = 
| Entypo | EvilIcons | Feather | FontAwesome | Foundation | Ionicons | MaterialCommunityIcons | MaterialIcons | Octicons | SimpleLineIcons | Zocial;

let make = (~style=?, ~iconType: option(iconType)=?, ~name: option(string)=?, ~ios: option(string)=?, ~android: option(string)=?, ~active: option(bool)=?, children) =>
